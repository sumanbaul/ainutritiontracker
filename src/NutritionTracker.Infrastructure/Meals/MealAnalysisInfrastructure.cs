...
public sealed class S3MealImageStorage(IOptions<MealAnalysisOptions> options, IHttpClientFactory clients) : IMealImageStorage
{
    ...
    public async Task<string> SaveAsync(ReadOnlyMemory<byte> image, string mimeType, CancellationToken ct)
    {
        ...
        var key = $"meal-images/{DateTime.UtcNow:yyyy/MM}/{Guid.NewGuid():N}{extension}";
