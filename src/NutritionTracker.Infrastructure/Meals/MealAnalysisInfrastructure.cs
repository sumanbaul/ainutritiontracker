...
{
    public async Task<string> SaveAsync(ReadOnlyMemory<byte> image, string mimeType, CancellationToken ct) { var extension = mimeType switch { "image/jpeg" => ".jpg", "image/png" => ".png", "image/webp" => ".webp", _ => throw new MealVisionImageValidationException("Unsupported image type.", 415) }; var root = Path.GetFullPath(options.Value.LocalStorageRoot); Directory.CreateDirectory(root); var key = $"{DateTime.UtcNow:yyyy/MM}/{Guid.NewGuid():N}{extension}"; var path = Path.GetFullPath(Path.Combine(root, key.Replace('/', Path.DirectorySeparatorChar))); if (!path.StartsWith(root, StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("Invalid generated storage path."); Directory.CreateDirectory(Path.GetDirectoryName(path)!); await File.WriteAllBytesAsync(path, image.ToArray(), ct); return key; }
    public Task DeleteAsync(string storageKey, CancellationToken ct) { ct.ThrowIfCancellationRequested(); var root = Path.GetFullPath(options.Value.LocalStorageRoot); var path = Path.GetFullPath(Path.Combine(root, storageKey.Replace('/', Path.DirectorySeparatorChar))); if (path.StartsWith(root, StringComparison.OrdinalIgnoreCase) && File.Exists(path)) File.Delete(path); return Task.CompletedTask; }
}
...
public sealed class MealAnalysisPipeline(NutritionTrackerDbContext db, IMealVisionAnalysisService vision, IMealImageStorage storage, IFoodMatcher matcher, IFoodNutritionCalculator nutrition) : IMealAnalysisPipeline
{
    ...
    public async Task<MealReviewResult?> GetReviewAsync(string userId, Guid mealId, CancellationToken ct) { var meal = await db.Meals.AsNoTracking().AsSplitQuery().Include(x => x.Items).Include(x => x.AnalysisRuns).SingleOrDefaultAsync(x => x.Id == mealId && x.UserId == userId && x.Status != MealStatus.Deleted, ct); return meal is null ? null : ToReview(meal); }
