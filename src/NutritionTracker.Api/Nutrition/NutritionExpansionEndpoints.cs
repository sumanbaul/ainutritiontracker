...
public sealed record RecipeRequest(string Name, string? Description, string? PreparationNotes, decimal YieldGrams, decimal ServingCount, bool IsSavedTemplate, IReadOnlyList<RecipeIngredientRequest> Ingredients, long? ExpectedVersion = null);
public sealed record HabitOperationRequest(decimal? Millilitres, DateTime? RecordedAtUtc, DateTime? StartedAtUtc, DateTime? EndedAtUtc, string? Type, TimeOnly? LocalTime, string? Timezone, bool? IsEnabled, string? ClientOperationId, long? ExpectedVersion = null);
...
public static class NutritionExpansionEndpoints
{
    ...
    private static async Task<IResult> UpdateRecipe(HttpContext c, NutritionTrackerDbContext db, Guid id, RecipeRequest r, CancellationToken ct) { var recipe = await Owned(db, User(c)).SingleOrDefaultAsync(x => x.Id == id, ct); if (recipe is null) return Results.NotFound(); if (r.ExpectedVersion is not null && recipe.Version != r.ExpectedVersion) return Conflict(recipe.Version); await Apply(recipe, r, db, ct); await db.SaveChangesAsync(ct); return Results.Ok(View(recipe)); }
    ...
    private static object View(Recipe r) { var total = r.Ingredients.Sum(i => i.Grams / 100m * i.Food.CaloriesPer100Grams); return new { r.Id, r.Name, r.Description, r.PreparationNotes, r.YieldGrams, r.ServingCount, r.IsSavedTemplate, r.Version, r.UpdatedAtUtc, CaloriesPerServing = decimal.Round(total / r.ServingCount, 1), Ingredients = r.Ingredients.Select(i => new { i.FoodId, Food = i.Food.DisplayName, i.Grams, i.Notes }) }; }
    ...
    private static async Task<IResult> PutReminder(HttpContext c, NutritionTrackerDbContext db, Guid id, HabitOperationRequest r, CancellationToken ct) { if (string.IsNullOrWhiteSpace(r.Type) || r.LocalTime is null || string.IsNullOrWhiteSpace(r.Timezone)) return Results.BadRequest(); var user = User(c); var existing = await db.ReminderPreferences.SingleOrDefaultAsync(x => x.Id == id && x.UserId == user, ct); if (existing is not null && r.ExpectedVersion is not null && existing.Version != r.ExpectedVersion) return Conflict(existing.Version); var item = existing ?? new ReminderPreference { Id = id, UserId = user }; item.Type = r.Type; item.LocalTime = r.LocalTime.Value; item.Timezone = r.Timezone; item.IsEnabled = r.IsEnabled ?? true; if (item.CreatedAtUtc == default) db.ReminderPreferences.Add(item); await db.SaveChangesAsync(ct); return Results.Ok(item); }
    private static IResult Conflict(long serverVersion) => Results.Problem(statusCode: 409, title: "The resource changed on another device.", extensions: new Dictionary<string, object?> { ["code"] = "concurrency_conflict", ["serverVersion"] = serverVersion });
